...
property :type, [String, Symbol],
          ...
          description: 'Vault server entropy type.'

property :options, Hash,
          default: lazy { default_vault_config_hcl(:entropy) },
          description: 'Vault server entropy configuration.'
...
load_current_value do
  current_value_does_not_exist! if vault_hcl_config_current_load(config_file).dig(:entropy, type).nil?
  options vault_hcl_config_current_load(config_file).dig(:entropy, type)
end
...
action :create do
  ...
  vault_hcl_config_resource.variables[:entropy] ||= []
  vault_hcl_config_resource.variables[:entropy].push(vault_hcl_resource_data)
end

action :delete do
  vault_hcl_config_resource_init

  vault_hcl_config_resource.variables[:entropy].delete(vault_hcl_resource_data)
end
