...
describe 'hashicorp-vault::default' do
  before do
    stub_command('test -L /opt/vault/0.6.3/vault').and_return(true)
    stub_command('getcap /opt/vault/0.6.3/vault|grep cap_ipc_lock+ep').and_return(false)
  end
  ...
  context 'with default node attributes' do
    ...
    it { expect(chef_run).to create_vault_installation('0.6.3') }
